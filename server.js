...
function startPartyGame(party) {
  ...
  party.members.forEach(m => {
    io.to(m.socketId).emit('game:start', { mode: party.mode, diff: party.diff });
  });
...
io.on('connection', socket => {
  ...
  socket.on('admin:command', async ({ cmd, target }) => {
    if (!socketUser || socketUser.username.toLowerCase() !== 'adminmaik') {
      return socket.emit('admin:error', 'Kein Admin-Zugriff.');
    }
